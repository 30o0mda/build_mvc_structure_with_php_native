...
<div
    ...
    <h2>{{ trans('admin.news') }} - {{ trans('admin.create_news') }}</h2>
    <a class="btn btn-info" href="{{aurl('news')}}">{{ trans('admin.news') }}</a>
</div>
<form method="post" action="{{aurl('news/store')}}" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-24">
            <div class="form-group">
                ...
                <textarea name="description" placeholder="{{trans('news.description')}}" class="form-control<?php echo !empty(get_error('description')) ? ' is-invalid' : ''; ?>">{{old('description')}}</textarea>
            </div>
        </div>
        <div class="col-md-24">
            <div class="form-group">
                <label for="content">{{trans('news.content')}}</label>
                <textarea name="content" id="content" placeholder="{{trans('news.content')}}" class="form-control<?php echo !empty(get_error('content')) ? ' is-invalid' : ''; ?>">{{old('content')}}</textarea>
            </div>
        </div>
    </div>
