...
$news3 = db_first('news', 'join categories on news.category_id = categories.id where news.category_id!="' . $news2['cat_id'] . '"
         order by id desc', "
         categories.name as cat_name,
         categories.id as cat_id,
         news .title,
      ...
      <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
        ...
        <div class="col-auto d-none d-lg-block">
          ...
          if (!empty($news1['image'])) {
            $img = url('storage/files/' . $news1['image']);
          } else {
      ...
      <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
        ...
        <div class="col-auto d-none d-lg-block">
          ...
          if (!empty($news2['image'])) {
            $img = url('storage/files/' . $news2['image']);
          } else {
            $img = url('assets/front/image/icon.png');
          }
          ?>
...
<div class="row g-5">
 ...
 <div class="col-auto d-none d-lg-block">
          ...
          ?>
          <img src="<?php echo  $img ; ?>" class="bd-placeholder-img" style="width: 100px; height: 100;">
        </div>
              <div class="col-lg-8">
                <h6 class="mb-0"><?php echo $news['title']; ?></h6>
                <small class="text-body-secondary"><?php echo  date('M d', strtotime($news['created_at'])) ; ?></small>
              </div>
            </a>
          </li>  
          <?php endwhile; ?>
      ...
      <div class="p-4">
        ...
        <ol class="list-unstyled mb-0">
          <?php while ($item = mysqli_fetch_assoc($years['query'])): 
            $news_year = date('Y', strtotime($item['created_at']));
            ?>
